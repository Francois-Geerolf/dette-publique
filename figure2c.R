...
plot_df <- data %>%
  transmute(
    ...
    `Taxe inflationniste (Md€)`   = taxe_inflationniste
  ) %>%
...
ggplot(plot_df, aes(date, value, color = variable)) +
  ...
  scale_color_manual(
    ...
    values = c(
      ...
      "Charge d'intérêts réelle (Md€)"   = "#10B981", # blue
      "Taxe inflationniste (Md€)"   = "#EF4444"  # green"
    )
  ) +
...
ggsave("figure2c.png", width = 1.25*6, height = 1.25*3.375, bg = "white")
ggsave("figure2c.pdf", width = 1.25*6, height = 1.25*3.375)
