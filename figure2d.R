...
plot_df <- data %>%
  transmute(
    ...
    `Taxe inflationniste (% du PIB)` = (taxe_inflationniste) / PIB
  ) %>%
  ...
  mutate(
    variable = factor(
      variable,
      levels = c("Taxe inflationniste (% du PIB)", "Charge d'intérêts nominale (% du PIB)", "Charge d'intérêts réelle (% du PIB)")
    )
  )
...
ggplot(plot_df, aes(x = date, y = value, color = variable)) +
  ...
  scale_color_manual(
    ...
    values = c(
      ...
      "Charge d'intérêts réelle (% du PIB)"   = "#10B981" , # blue
      "Taxe inflationniste (% du PIB)"   = "#EF4444"  # green"
    )
  ) +
  scale_y_continuous(
...
ggsave("figure2d.png", width = 1.25*6, height = 1.25*3.375, bg = "white")
ggsave("figure2d.pdf", width = 1.25*6, height = 1.25*3.375)
